...
使用本组件时，需要在项目中创建以下目录并放入对应图片：

```
public/
└── images/
    └── logo/
        ├── logo-full.png   ← 侧边栏展开时显示（建议尺寸 ≤ 160×40，透明背景）
        └── logo-icon.png   ← 侧边栏折叠时显示（建议尺寸 40×40，透明背景）
```

> **图片缺失时区域留空：** 如果图片路径不存在或加载失败，Logo 区域（56px 高）仍然保留，内部不显示任何内容，菜单正常显示。把图片放入对应路径后刷新页面即可看到效果，无需改代码。
